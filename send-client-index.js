...
Zepto(function ($) {
    ...
    /**
     *  通过缓存获取运营位 或者公告的 公共方法，，从之前的公共方法拆分出来
     *  @method getPromotionOrNoticeCache
     *  @param  {String}                   cityCode 当前城市code
     *  @param  {String}                   pageId 当前页面pageID
     *  @return {null}                            无返回值
     */
    function getPromotionOrNoticeCache(getSessionName,cityCode, pageId,spmId) {
        var internalCacheName = getSessionName +cityCode;
     ...
     *  @param  {String}     spmId          公告对应的埋点
     *  @return {null}                     无返回值
     */
    function getPromotion(indexPromotion, cityCode, cacheName, internalPageId,spmId) {
        if (indexPromotion || indexPromotion === null) {
            if (indexPromotion === null) {
                $(".promotion-txt").hide();
                return;
            } else {
                if (!Array.isArray(indexPromotion)){
                    indexPromotion = JSON.parse(indexPromotion);
                }
            }
            setPromotionImg(indexPromotion,spmId)
        } else {
            promotion(cityCode, internalPageId, spmId, cacheName);
        }
    }
    // 获取公告
    /**
     ...
     *  @return {null}                       无返回值
     */
    function getIndexNotice(indexNotice, cityCode, cacheName, internalPageId,spmId) {
        if (indexNotice || indexNotice === null) {
            ...
            getNotice(cityCode, internalPageId, spmId, cacheName);
        }
    }
    //获取城市
